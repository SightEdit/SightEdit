...
export const filterDefinitions: Record<FilterType, FilterDefinition> = {
  none: {
    name: 'Original',
    css: 'none'
  },
...
function applyColorMatrix(canvas: HTMLCanvasElement, matrix: number[]): void {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    data[i] = Math.min(255, Math.max(0, 
      r * matrix[0] + g * matrix[1] + b * matrix[2] + matrix[3] * 255 + matrix[4]
    ));
    data[i + 1] = Math.min(255, Math.max(0,
      r * matrix[5] + g * matrix[6] + b * matrix[7] + matrix[8] * 255 + matrix[9]
    ));
    data[i + 2] = Math.min(255, Math.max(0,
      r * matrix[10] + g * matrix[11] + b * matrix[12] + matrix[13] * 255 + matrix[14]
    ));
  }
...
export class ImageFilter {
  ...
  private setupUI(): void {
    ...
    // Event listeners
    const select = this.container.querySelector('.sightedit-filter-select') as HTMLSelectElement;
    select?.addEventListener('change', () => {
      this.currentFilter = select.value as FilterType;
      this.render();
    });
    ...
    this.container.querySelectorAll('input[type="range"]').forEach(input => {
      input.addEventListener('input', (e) => {
        ...
        const adjust = target.dataset.adjust as keyof typeof this.adjustments;
  ...
  getFilteredDataURL(format = 'image/jpeg', quality = 0.92): string {
