import { BaseEditor, EditMode, HTMLSanitizer } from '@sightedit/core';
...
export class MarkdownEditor extends BaseEditor {
  ...
  private pluginOptions: MarkdownPluginOptions;
  ...
  render(): void {
    ...
    this.container = modal.querySelector('.sightedit-markdown-container') as HTMLElement;
  ...
  private handleToolbarAction(action: string): void {
    switch (action) {
      ...
      case 'heading': this.cycleHeading(); break;
  ...
  private updatePreview(): void {
    ...
    try {
      ...
      const sanitizedHTML = HTMLSanitizer.sanitize(html, {
        ...
        maxLength: 100000 // 100KB limit for markdown preview
      }, true); // Use strict mode for markdown content
      
      this.previewPane.innerHTML = sanitizedHTML;
    } catch (error) {
      console.error('Failed to update markdown preview:', error);
      this.previewPane.innerHTML = '<p>Preview unavailable - content contains unsafe elements</p>';
    }
  }
...
\`\`\`</code> - Code block</li>
  ...
  applyValue(value: string): void {
    ...
    try {
      ...
      const renderedHTML = this.renderer.render(value);
      const sanitizedHTML = HTMLSanitizer.sanitizeUserContent(renderedHTML);
      
      // Update original element with sanitized content
      this.element.innerHTML = sanitizedHTML;
    } catch (error) {
      console.error('Failed to apply markdown value:', error);
